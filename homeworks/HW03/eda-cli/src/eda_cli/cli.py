...
def report(
    ...
    min_missing_share: float = typer.Option(0.05, help = "Порог доли пропусков"),
    top_k_categories: int = typer.Option(5, help = "Top-значений для категориальных признаков")
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if (quality_flags['list_problems']):
            f.write("## Проблемный список\n\n")
            f.write(f"- Столбцы с пропусками больше порога {min_missing_share} доли пропусков: **{quality_flags['list_problems']}**\n")
        ...
        f.write(f" - top-значений: **{top_k_categories}**\n\n")
        if not top_cats:
...
def head(
    ...
    sep: str = typer.Option(",", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", help="Кодировка файла."),
    n: int = typer.Option(5, help="Вывести первых n строк с csv")
) -> None:
    """
    Напечатать n (по умолчанию 5) первых строк из датасета:
    """
    df = _load_csv(Path(path), sep=sep, encoding=encoding)

    typer.echo(f"\nКоличество строк выведено {n}")
